...
public class CustomersDao {
    ...
    public boolean registerCustomerQuery(Customers customer){
        String query = "INSER INTO customers (id, full_name, address, telephone, email, created, updated)"
                + "VALUES (?.?.?.?.?.?.?)";
    ...
    //Listar Customer
    public List lsitCustomerQuery(String value){
        ...
        try{
            ...
            while(rs.next()){
                ...
                list_customers.add(customer);//pasar toda la informacion a la lista
            }      
        }catch(SQLException e){
    ...
    public boolean updateCustomerQuery(Customers customer){
         ...
         try{
             ...
             JOptionPane.showMessageDialog(null, "Error al modificar los datos de cliente  "+ e);
             return false;
         }
    }
